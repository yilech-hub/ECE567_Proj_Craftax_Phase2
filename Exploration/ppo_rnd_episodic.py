...
def make_train(config):
    ...
    def train(rng):
        ...
        if config["USE_RND"]:
            ...
            # Simplified NGU: episodic ring buffer + running variance stats
            ex_state["ep_buffer"] = jnp.zeros(
                (config["NUM_ENVS"], config["MAX_MEM"], config["RND_OUTPUT_SIZE"]),
                dtype=jnp.float32,
            )
            ex_state["ep_count"] = jnp.zeros((config["NUM_ENVS"],), dtype=jnp.int32)
            ex_state["rnd_global_var"] = jnp.float32(1.0)
            ex_state["ep_var"] = jnp.float32(1.0)
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                if config["USE_RND"]:
                    # ---- RND target + predictor forward (target also reused as episodic embedding) ----
                    ...
                    mse = jnp.square(error).mean(axis=-1)  # (NUM_ENVS,)

                    eps_num = 1e-8

                    # ---- r_global (optionally std-normalized) ----
                    if config["NORMALIZE_RND"]:
                        r_global = mse / (
                            jnp.sqrt(ex_state["rnd_global_var"]) + eps_num
                        )
                    else:
                        r_global = mse

                    # ---- Episodic novelty (k-NN over target embeddings in ring buffer) ----
                    if config["EPISODIC_BONUS"]:
                        e_t = random_pred  # reuse frozen target embedding
                        # pairwise L2^2 to every slot in the ring buffer
                        dist_sq = jnp.square(
                            ex_state["ep_buffer"] - e_t[:, None, :]
                        ).sum(axis=-1)  # (NUM_ENVS, MAX_MEM)
                        # mask slots that haven't been written yet this episode
                        valid_count = jnp.minimum(
                            ex_state["ep_count"], config["MAX_MEM"]
                        )  # (NUM_ENVS,)
                        valid_mask = (
                            jnp.arange(config["MAX_MEM"])[None, :]
                            < valid_count[:, None]
                        )
                        dist_sq_masked = jnp.where(valid_mask, dist_sq, jnp.inf)
                        # top-k smallest (== largest of negated)
                        neg_topk, _ = jax.lax.top_k(
                            -dist_sq_masked, config["K_NEIGHBORS"]
                        )
                        topk_dist = -neg_topk  # (NUM_ENVS, K)
                        # average ONLY over finite entries (handles ep_count < K case)
                        is_valid_in_topk = jnp.isfinite(topk_dist)
                        topk_valid_count = is_valid_in_topk.sum(axis=-1)  # (NUM_ENVS,)
                        topk_sum = jnp.where(
                            is_valid_in_topk, topk_dist, 0.0
                        ).sum(axis=-1)
                        d_t = jnp.where(
                            topk_valid_count > 0,
                            topk_sum / jnp.maximum(topk_valid_count, 1),
                            1.0,  # empty buffer -> max novelty
                        )
                        r_ep = d_t / (jnp.sqrt(ex_state["ep_var"]) + eps_num)
                        r_ep = r_ep * (1 - done)  # align mask with r_global

                        if config["COMBINATION_MODE"] == "multiply":
                            r_i = r_global * r_ep
                        elif config["COMBINATION_MODE"] == "add":
                            r_i = r_global + r_ep
                        elif config["COMBINATION_MODE"] == "anneal":
                            # linear schedule on r_global weight; r_ep stays full.
                            # progress is computed per-update, so training-length-relative.
                            progress = update_step / jnp.float32(config["NUM_UPDATES"])
                            global_weight = jnp.maximum(0.0, 1.0 - progress)
                            r_i = global_weight * r_global + r_ep
                        elif config["COMBINATION_MODE"] == "episodic_only":
                            r_i = r_ep
                        else:
                            raise ValueError(
                                f"Unknown COMBINATION_MODE: {config['COMBINATION_MODE']}"
                            )

                        # ring-buffer write + reset on done
                        write_idx = ex_state["ep_count"] % config["MAX_MEM"]
                        ep_buffer_new = ex_state["ep_buffer"].at[
                            jnp.arange(config["NUM_ENVS"]), write_idx
                        ].set(e_t)
                        ep_count_new = jnp.where(done, 0, ex_state["ep_count"] + 1)

                        # EMA running variance update (batch of NUM_ENVS samples)
                        alpha = config["EMA_ALPHA"]
                        rnd_global_var_new = (
                            (1 - alpha) * ex_state["rnd_global_var"]
                            + alpha * mse.var()
                        )
                        ep_var_new = (
                            (1 - alpha) * ex_state["ep_var"] + alpha * d_t.var()
                        )

                        ex_state = {
                            **ex_state,
                            "ep_buffer": ep_buffer_new,
                            "ep_count": ep_count_new,
                            "rnd_global_var": rnd_global_var_new,
                            "ep_var": ep_var_new,
                        }
                    else:
                        r_i = r_global
                        # Still update rnd_global_var if normalization is on
                        if config["NORMALIZE_RND"]:
                            alpha = config["EMA_ALPHA"]
                            rnd_global_var_new = (
                                (1 - alpha) * ex_state["rnd_global_var"]
                    ...
                    reward_i = r_i * config["RND_REWARD_COEFF"]
...
def run_ppo(config):
    ...
    if config["USE_WANDB"]:
        suffix = ""
        if config["EPISODIC_BONUS"]:
            suffix = f"_EP_{config['COMBINATION_MODE']}"
        if not config["NORMALIZE_RND"]:
            suffix += "_noNorm"
        suffix += f"_c{config['RND_REWARD_COEFF']}"
        wandb.init(
...
if __name__ == "__main__":
    ...
    # Episodic + normalization extensions (Simplified NGU)
    parser.add_argument(
        "--episodic_bonus", action=argparse.BooleanOptionalAction, default=True
    )
    parser.add_argument(
        "--combination_mode", type=str, default="multiply",
        choices=["multiply", "add", "anneal", "episodic_only"],
    )
    parser.add_argument(
        "--normalize_rnd", action=argparse.BooleanOptionalAction, default=True
    )
    parser.add_argument("--k_neighbors", type=int, default=10)
    parser.add_argument("--max_mem", type=int, default=1024)
    parser.add_argument("--ema_alpha", type=float, default=0.001)
