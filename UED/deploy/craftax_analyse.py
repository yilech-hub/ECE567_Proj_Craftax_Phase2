...
def do_main(env='minigrid'):
    if env == 'jaxnav-single':
        ...
        CLEAN_NAMES = {
                # "ncc_minigrid": "NCC-Learn",
                # "ncc_reg_minigrid": "NCC-Reg",
                # "sfl_minigrid": "SFL",
                # "dr_minigrid": "DR",
                # "plr_minigrid": "PLR",
                "ncc": "NCC-Learn",
    ...
    # GROUPS = ["ncc_minigrid", "ncc_reg_minigrid", "sfl_minigrid", "dr_minigrid", "plr_minigrid"]
    # GROUPS = [
    #     "rsim_accel_maxmc_fixed_single_v4",
    ...
    if env in ["minigrid", "minigrid_256"]:
    ...
    def get_vals(group, seed_num):
        ...
        for seed in range(SEEDS):
            ...
            win_rates = jnp.array(df['returns'])# jnp.array(df['win-rates'])
    ...
    for i in range(len(bar_x)):
        plt.bar(bar_x[i], bar_y[i], yerr=bar_std[i] / np.sqrt(SEEDS) * 100, label=CLEAN_NAMES[bar_x[i]],
    ...
    for group_name, cvar in cvar_results_per_group.items():
        label = CLEAN_NAMES[group_name]
        print(label, GROUP_TO_COLOUR[CLEAN_NAMES[group_name]])
        plot_mean_std_xy(
            percentages, cvar.mean(axis=0), cvar.std(axis=0) / np.sqrt(SEEDS), plt.gca(), label=CLEAN_NAMES[group_name], marker='o', linewidth=4,
            color=GROUP_TO_COLOUR[CLEAN_NAMES[group_name]]
        )
    _annotate_and_decorate_axis(plt.gca(), xlabel=r'$\alpha$', ylabel=r'Avg Return on worst-case $\alpha$% levels', labelsize='x-large', ticklabelsize='x-large', wrect=10, hrect=10, legend=True, OURS=OURS)
    plt.xscale('log')
    if env == 'jaxnav-single' or env == "craftax":
    ...
    BASE_MODEL = 'rsim_learnability_single_v4' if env == 'jaxnav-single' else 'ncc'
